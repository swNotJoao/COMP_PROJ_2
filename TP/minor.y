...
%type<n> instr	elifs	else	expr	exprs	block	main lbl

%token FARGS CHARS INTS ADDR VAR ARGS DECL NIL TPAND GOTO LABEL
...
instrs	:			{ $$ = nilNode(NIL); }
	...
	| instrs lbl { $$ = binNode(';', $1, $2); }
	;

lbl : ID ':' {$$ = uniNode(LABEL, TID($1));}
		;
...
expr	: chars			{ $$ = $1; }
	...
	| expr '&' expr		{ $$ = binNode('&', $1, $3); $$->info = isInt($$, "&"); }
	| expr '|' expr		{ $$ = binNode('|', $1, $3); $$->info = isInt($$, "|"); }
	| expr TPAND expr {$$ = binNode(TPAND, $1, $3);}
