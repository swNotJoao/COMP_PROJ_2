/*
generated at Fri May 22 18:45:50 2020
...
/* 22 */	"instr: NIL",
/* 23 */	"instr: STMT(NIL,instr)",
/* 24 */	"instr: STMT(instr,instr)",
/* 25 */	"instr: print",
/* 26 */	"instr: expr",
/* 27 */	"instr: FI(THEN(if,elif),instr)",
/* 28 */	"elif: ELIF(elif,if)",
/* 29 */	"elif: NIL",
/* 30 */	"instr: FI(THEN(if,NIL),instr)",
/* 31 */	"if: IF(cond,instr)",
/* 32 */	"instr: FI(THEN(instr,NIL),NIL)",
/* 33 */	"instr: IF(cond,instr)",
/* 34 */	"cond: expr",
/* 35 */	"print: PSTMT(ID)",
/* 36 */	"print: PSTMT(chars)",
/* 37 */	"print: PSTMT(expr)",
/* 38 */	"print: PSTMT(CHARS(NIL,INT))",
/* 39 */	"chars: CHARS(NIL,chars)",
/* 40 */	"chars: INT",
/* 41 */	"chars: CHAR",
/* 42 */	"chars: STR",
/* 43 */	"chars: CHARS(chars,chars)",
/* 44 */	"expr: CHARS(NIL,INT)",
/* 45 */	"expr: CHARS(NIL,CHAR)",
/* 46 */	"expr: ID",
/* 47 */	"expr: READOP",
/* 48 */	"expr: ADDR(ID)",
/* 49 */	"expr: ADDR(IND(expr,ID))",
/* 50 */	"expr: UMINUS(expr)",
/* 51 */	"expr: ADD(expr,expr)",
/* 52 */	"expr: SUB(expr,expr)",
/* 53 */	"expr: MUL(expr,expr)",
/* 54 */	"expr: DIV(expr,expr)",
/* 55 */	"expr: MOD(expr,expr)",
/* 56 */	"expr: ISEQ(expr,expr)",
/* 57 */	"expr: NE(expr,expr)",
/* 58 */	"expr: LT(expr,expr)",
/* 59 */	"expr: LE(expr,expr)",
/* 60 */	"expr: GT(expr,expr)",
/* 61 */	"expr: GE(expr,expr)",
/* 62 */	"expr: AND(and,expr)",
/* 63 */	"and: expr",
/* 64 */	"expr: OR(or,expr)",
/* 65 */	"or: expr",
/* 66 */	"expr: UMINUS(expr)",
...
static void yyclosure_fvar(NODEPTR_TYPE a, int c) {
	...
	yytrace(a, 18, c + 1, p->cost[yyfvars_NT]);
	if (c + 1 < p->cost[yyfvars_NT]) {
		p->cost[yyfvars_NT] = c + 1;
		p->rule.yyfvars = 3;
	}
}
...
static void yyclosure_expr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 65, c + 1, p->cost[yyor_NT]);
	if (c + 1 < p->cost[yyor_NT]) {
		p->cost[yyor_NT] = c + 1;
		p->rule.yyor = 1;
	}
	yytrace(a, 63, c + 1, p->cost[yyand_NT]);
	if (c + 1 < p->cost[yyand_NT]) {
		p->cost[yyand_NT] = c + 1;
		p->rule.yyand = 1;
	}
	yytrace(a, 34, c + 1, p->cost[yycond_NT]);
...
static void yylabel(NODEPTR_TYPE a, NODEPTR_TYPE u) {
	...
	case 33: /* PSTMT */
		...
		if (	/* print: PSTMT(CHARS(NIL,INT)) */
			...
			c = 1;
			yytrace(a, 38, c + 0, p->cost[yyprint_NT]);
			if (c + 0 < p->cost[yyprint_NT]) {
				p->cost[yyprint_NT] = c + 0;
				p->rule.yyprint = 4;
				yyclosure_print(a, c + 0);
	...
	case 63: /* READOP */
		/* expr: READOP */
		yytrace(a, 47, 0 + 0, p->cost[yyexpr_NT]);
	...
	case 259: /* ID */
		/* expr: ID */
		yytrace(a, 46, 1 + 0, p->cost[yyexpr_NT]);
	...
	case 260: /* STR */
		/* chars: STR */
		yytrace(a, 42, 0 + 0, p->cost[yychars_NT]);
		if (0 + 0 < p->cost[yychars_NT]) {
			p->cost[yychars_NT] = 0 + 0;
			p->rule.yychars = 4;
		}
	...
	case 272: /* IF */
		...
		yytrace(a, 31, c + 0, p->cost[yyif_NT]);
		...
		yytrace(a, 33, c + 0, p->cost[yyinstr_NT]);
		if (c + 0 < p->cost[yyinstr_NT]) {
			p->cost[yyinstr_NT] = c + 0;
			p->rule.yyinstr = 11;
		}
	...
	case 274: /* FI */
		...
		if (	/* instr: FI(THEN(if,elif),instr) */
			...
			yytrace(a, 27, c + 0, p->cost[yyinstr_NT]);
			if (c + 0 < p->cost[yyinstr_NT]) {
				p->cost[yyinstr_NT] = c + 0;
				p->rule.yyinstr = 8;
			}
	...
	case 275: /* ELIF */
		...
		yytrace(a, 28, c + 0, p->cost[yyelif_NT]);
	...
	case 293: /* NIL */
		...
		/* instr: NIL */
		yytrace(a, 22, 0 + 0, p->cost[yyinstr_NT]);
		if (0 + 0 < p->cost[yyinstr_NT]) {
			p->cost[yyinstr_NT] = 0 + 0;
			p->rule.yyinstr = 3;
		}
		/* elif: NIL */
		yytrace(a, 29, 2 + 0, p->cost[yyelif_NT]);
	...
	case 294: /* EQ */
		...
		if (	/* expr: EQ(chars,ID) */
			...
			c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yychars_NT] + 2;
...
static void yykids(NODEPTR_TYPE p, int eruleno, NODEPTR_TYPE kids[]) {
	...
	case 64: /* expr: OR(or,expr) */
	case 62: /* expr: AND(and,expr) */
	case 61: /* expr: GE(expr,expr) */
	case 60: /* expr: GT(expr,expr) */
	case 59: /* expr: LE(expr,expr) */
	case 58: /* expr: LT(expr,expr) */
	case 57: /* expr: NE(expr,expr) */
	case 56: /* expr: ISEQ(expr,expr) */
	case 55: /* expr: MOD(expr,expr) */
	case 54: /* expr: DIV(expr,expr) */
	case 53: /* expr: MUL(expr,expr) */
	case 52: /* expr: SUB(expr,expr) */
	case 51: /* expr: ADD(expr,expr) */
	case 43: /* chars: CHARS(chars,chars) */
	case 33: /* instr: IF(cond,instr) */
	case 31: /* if: IF(cond,instr) */
	case 28: /* elif: ELIF(elif,if) */
	case 24: /* instr: STMT(instr,instr) */
	...
	case 1: /* file: PROGRAM(decls,main) */
		...
		break;
	case 48: /* expr: ADDR(ID) */
	case 47: /* expr: READOP */
	case 46: /* expr: ID */
	case 45: /* expr: CHARS(NIL,CHAR) */
	case 44: /* expr: CHARS(NIL,INT) */
	case 42: /* chars: STR */
	case 41: /* chars: CHAR */
	case 40: /* chars: INT */
	case 38: /* print: PSTMT(CHARS(NIL,INT)) */
	case 35: /* print: PSTMT(ID) */
	case 29: /* elif: NIL */
	case 22: /* instr: NIL */
	...
	case 4: /* decls: VAR(ARRAY(ID,INTS(INT,eqints))) */
		...
		break;
	case 71: /* expr: IND(expr,ID) */
	case 70: /* expr: EQ(expr,ID) */
	case 69: /* expr: EQ(chars,ID) */
	case 67: /* expr: NOT(expr) */
	case 66: /* expr: UMINUS(expr) */
	case 50: /* expr: UMINUS(expr) */
	case 37: /* print: PSTMT(expr) */
	case 36: /* print: PSTMT(chars) */
	...
	case 10: /* decls: VAR(STRING(ID,chars)) */
		...
		break;
	case 39: /* chars: CHARS(NIL,chars) */
	case 23: /* instr: STMT(NIL,instr) */
	...
	case 18: /* fvars: fvar */
		kids[0] = p;
		break;
	case 27: /* instr: FI(THEN(if,elif),instr) */
		kids[0] = LEFT_CHILD(LEFT_CHILD(p));
		kids[1] = RIGHT_CHILD(LEFT_CHILD(p));
		kids[2] = RIGHT_CHILD(p);
		break;
	case 30: /* instr: FI(THEN(if,NIL),instr) */
		kids[0] = LEFT_CHILD(LEFT_CHILD(p));
		kids[1] = RIGHT_CHILD(p);
		break;
	case 49: /* expr: ADDR(IND(expr,ID)) */
	case 32: /* instr: FI(THEN(instr,NIL),NIL) */
		kids[0] = LEFT_CHILD(LEFT_CHILD(p));
		break;
	case 72: /* expr: EQ(expr,IND(expr,ID)) */
		kids[0] = LEFT_CHILD(p);
...
{locals[localvarind].id = p->SUB(0)->value.s;
		...
		break;
	case 20: /* fvar: ARRAY(ID,INTS(INT,NIL)) */
		fprintf(stderr, "0x%p: line 127: fvar: ARRAY(ID,INTS(INT,NIL))\n",(void*)p);
#line 127 "minor.brg"
{ locals[localvarind].id = p->SUB(0)->value.s;
		...
		break;
	case 21: /* fvar: STRING(ID,NIL) */
		fprintf(stderr, "0x%p: line 134: fvar: STRING(ID,NIL)\n",(void*)p);
#line 134 "minor.brg"
{locals[localvarind].id = p->SUB(0)->value.s;
                        accoffset-=4;
                        locals[localvarind].offset = accoffset;
		...
		break;
	case 22: /* instr: NIL */
		fprintf(stderr, "0x%p: line 140: instr: NIL\n",(void*)p);
#line 140 "minor.brg"

		break;
	case 23: /* instr: STMT(NIL,instr) */
		fprintf(stderr, "0x%p: line 141: instr: STMT(NIL,instr)\n",(void*)p);
#line 141 "minor.brg"

		break;
	case 24: /* instr: STMT(instr,instr) */
		fprintf(stderr, "0x%p: line 142: instr: STMT(instr,instr)\n",(void*)p);
#line 142 "minor.brg"

		break;
	case 25: /* instr: print */
		fprintf(stderr, "0x%p: line 143: instr: print\n",(void*)p);
#line 143 "minor.brg"

		break;
	case 26: /* instr: expr */
		fprintf(stderr, "0x%p: line 144: instr: expr\n",(void*)p);
#line 144 "minor.brg"

		break;
	case 27: /* instr: FI(THEN(if,elif),instr) */
		fprintf(stderr, "0x%p: line 146: instr: FI(THEN(if,elif),instr)\n",(void*)p);
#line 146 "minor.brg"
{ fprintf(yyout, pfLABEL,
                                      mklbl(LEFT_CHILD(p)->SUB(0)->place)); }
		break;
	case 28: /* elif: ELIF(elif,if) */
		fprintf(stderr, "0x%p: line 148: elif: ELIF(elif,if)\n",(void*)p);
#line 148 "minor.brg"
{ p->place = ++lbl; fprintf(yyout, pfJMP pfLABEL,
				                mklbl(RIGHT_CHILD(p)->place), mklbl(p->place)); }
		break;
	case 29: /* elif: NIL */
		fprintf(stderr, "0x%p: line 150: elif: NIL\n",(void*)p);
#line 150 "minor.brg"
{}
		break;
	case 30: /* instr: FI(THEN(if,NIL),instr) */
		fprintf(stderr, "0x%p: line 152: instr: FI(THEN(if,NIL),instr)\n",(void*)p);
#line 152 "minor.brg"
{ fprintf(yyout, pfLABEL,
                                      mklbl(LEFT_CHILD(p)->SUB(0)->place)); }
		break;
	case 31: /* if: IF(cond,instr) */
		fprintf(stderr, "0x%p: line 154: if: IF(cond,instr)\n",(void*)p);
#line 154 "minor.brg"
{p->place = ++lbl; fprintf(yyout, pfJMP pfLABEL,
				            mklbl(p->place), mklbl(LEFT_CHILD(p)->place)); }
		break;
	case 32: /* instr: FI(THEN(instr,NIL),NIL) */
		fprintf(stderr, "0x%p: line 157: instr: FI(THEN(instr,NIL),NIL)\n",(void*)p);
#line 157 "minor.brg"
{/*Simple if*/}
		break;
	case 33: /* instr: IF(cond,instr) */
		fprintf(stderr, "0x%p: line 158: instr: IF(cond,instr)\n",(void*)p);
#line 158 "minor.brg"
{ fprintf(yyout, pfLABEL, mklbl(LEFT_CHILD(p)->place)); }
		break;
	case 34: /* cond: expr */
		fprintf(stderr, "0x%p: line 159: cond: expr\n",(void*)p);
#line 159 "minor.brg"
{ p->place = ++lbl; fprintf(yyout, pfJZ, mklbl(p->place)); }
		break;
	case 35: /* print: PSTMT(ID) */
		fprintf(stderr, "0x%p: line 161: print: PSTMT(ID)\n",(void*)p);
#line 161 "minor.brg"
{ret = getOffset(p->SUB(0)->value.s);
            if(ret){
            switch(p->SUB(0)->info){
              case tINT:
		...
		break;
	case 36: /* print: PSTMT(chars) */
		fprintf(stderr, "0x%p: line 192: print: PSTMT(chars)\n",(void*)p);
#line 192 "minor.brg"
{
		...
		break;
	case 37: /* print: PSTMT(expr) */
		fprintf(stderr, "0x%p: line 200: print: PSTMT(expr)\n",(void*)p);
#line 200 "minor.brg"
{fprintf(yyout, pfTEXT pfCALL pfTRASH, "_printi", 4);}
		break;
	case 38: /* print: PSTMT(CHARS(NIL,INT)) */
		fprintf(stderr, "0x%p: line 201: print: PSTMT(CHARS(NIL,INT))\n",(void*)p);
#line 201 "minor.brg"
{fprintf(yyout, pfTEXT pfIMM pfCALL pfTRASH, p->SUB(0)->SUB(1)->value.i, "_printi", 4);}
		break;
	case 39: /* chars: CHARS(NIL,chars) */
		fprintf(stderr, "0x%p: line 203: chars: CHARS(NIL,chars)\n",(void*)p);
#line 203 "minor.brg"
{}
		break;
	case 40: /* chars: INT */
		fprintf(stderr, "0x%p: line 204: chars: INT\n",(void*)p);
#line 204 "minor.brg"
{ tmpStr[tmpStrInd++] = p->value.i; toPrint = tmpStr;}
		break;
	case 41: /* chars: CHAR */
		fprintf(stderr, "0x%p: line 205: chars: CHAR\n",(void*)p);
#line 205 "minor.brg"
{ tmpStr[tmpStrInd++] = p->value.i; toPrint = tmpStr;}
		break;
	case 42: /* chars: STR */
		fprintf(stderr, "0x%p: line 206: chars: STR\n",(void*)p);
#line 206 "minor.brg"
{ strcpy(&tmpStr[tmpStrInd], p->value.s); tmpStrInd = tmpStrInd + strlen(p->value.s); toPrint = tmpStr;}
		break;
	case 43: /* chars: CHARS(chars,chars) */
		fprintf(stderr, "0x%p: line 207: chars: CHARS(chars,chars)\n",(void*)p);
#line 207 "minor.brg"
{}
		break;
	case 44: /* expr: CHARS(NIL,INT) */
		fprintf(stderr, "0x%p: line 209: expr: CHARS(NIL,INT)\n",(void*)p);
#line 209 "minor.brg"
{fprintf(yyout, pfIMM, p->SUB(1)->value.i);}
		break;
	case 45: /* expr: CHARS(NIL,CHAR) */
		fprintf(stderr, "0x%p: line 210: expr: CHARS(NIL,CHAR)\n",(void*)p);
#line 210 "minor.brg"
{fprintf(yyout, pfIMM, p->SUB(1)->value.i); }
		break;
	case 46: /* expr: ID */
		fprintf(stderr, "0x%p: line 211: expr: ID\n",(void*)p);
#line 211 "minor.brg"
{ret = getOffset(p->value.s);
            if(ret){
              fprintf(yyout, pfIMM "push ebp\n"
              pfADD pfPOP "push dword [eax]\n", ret);
            }
		...
		break;
	case 47: /* expr: READOP */
		fprintf(stderr, "0x%p: line 220: expr: READOP\n",(void*)p);
#line 220 "minor.brg"
{fprintf(yyout, pfCALL pfPUSH, "_readi"); }
		break;
	case 48: /* expr: ADDR(ID) */
		fprintf(stderr, "0x%p: line 222: expr: ADDR(ID)\n",(void*)p);
#line 222 "minor.brg"
{ fprintf(yyout, pfADDR, p->SUB(0)->value.s); }
		break;
	case 49: /* expr: ADDR(IND(expr,ID)) */
		fprintf(stderr, "0x%p: line 223: expr: ADDR(IND(expr,ID))\n",(void*)p);
#line 223 "minor.brg"
{ switch(p->SUB(0)->SUB(1)->info){
		...
		break;
	case 50: /* expr: UMINUS(expr) */
		fprintf(stderr, "0x%p: line 233: expr: UMINUS(expr)\n",(void*)p);
#line 233 "minor.brg"
{ fprintf(yyout, pfIMM pfMUL, -1); }
		break;
	case 51: /* expr: ADD(expr,expr) */
		fprintf(stderr, "0x%p: line 234: expr: ADD(expr,expr)\n",(void*)p);
#line 234 "minor.brg"
{ fprintf(yyout, pfADD); }
		break;
	case 52: /* expr: SUB(expr,expr) */
		fprintf(stderr, "0x%p: line 235: expr: SUB(expr,expr)\n",(void*)p);
#line 235 "minor.brg"
{ fprintf(yyout, pfSUB); }
		break;
	case 53: /* expr: MUL(expr,expr) */
		fprintf(stderr, "0x%p: line 236: expr: MUL(expr,expr)\n",(void*)p);
#line 236 "minor.brg"
{ fprintf(yyout, pfMUL); }
		break;
	case 54: /* expr: DIV(expr,expr) */
		fprintf(stderr, "0x%p: line 237: expr: DIV(expr,expr)\n",(void*)p);
#line 237 "minor.brg"
{ fprintf(yyout, pfDIV); }
		break;
	case 55: /* expr: MOD(expr,expr) */
		fprintf(stderr, "0x%p: line 238: expr: MOD(expr,expr)\n",(void*)p);
#line 238 "minor.brg"
{ fprintf(yyout, pfMOD); }
		break;
	case 56: /* expr: ISEQ(expr,expr) */
		fprintf(stderr, "0x%p: line 239: expr: ISEQ(expr,expr)\n",(void*)p);
#line 239 "minor.brg"
{ fprintf(yyout, pfEQ); }
		break;
	case 57: /* expr: NE(expr,expr) */
		fprintf(stderr, "0x%p: line 240: expr: NE(expr,expr)\n",(void*)p);
#line 240 "minor.brg"
{ fprintf(yyout, pfNE); }
		break;
	case 58: /* expr: LT(expr,expr) */
		fprintf(stderr, "0x%p: line 241: expr: LT(expr,expr)\n",(void*)p);
#line 241 "minor.brg"
{ fprintf(yyout, pfLT); }
		break;
	case 59: /* expr: LE(expr,expr) */
		fprintf(stderr, "0x%p: line 242: expr: LE(expr,expr)\n",(void*)p);
#line 242 "minor.brg"
{ fprintf(yyout, pfLE); }
		break;
	case 60: /* expr: GT(expr,expr) */
		fprintf(stderr, "0x%p: line 243: expr: GT(expr,expr)\n",(void*)p);
#line 243 "minor.brg"
{ fprintf(yyout, pfGT); }
		break;
	case 61: /* expr: GE(expr,expr) */
		fprintf(stderr, "0x%p: line 244: expr: GE(expr,expr)\n",(void*)p);
#line 244 "minor.brg"
{ fprintf(yyout, pfGE); }
		break;
	case 62: /* expr: AND(and,expr) */
		fprintf(stderr, "0x%p: line 245: expr: AND(and,expr)\n",(void*)p);
#line 245 "minor.brg"
{ fprintf(yyout, pfLABEL, mklbl(LEFT_CHILD(p)->place)); }
		break;
	case 63: /* and: expr */
		fprintf(stderr, "0x%p: line 246: and: expr\n",(void*)p);
#line 246 "minor.brg"
{ p->place = ++lbl; fprintf(yyout, pfDUP pfJZ pfTRASH, mklbl(p->place), pfWORD); }
		break;
	case 64: /* expr: OR(or,expr) */
		fprintf(stderr, "0x%p: line 247: expr: OR(or,expr)\n",(void*)p);
#line 247 "minor.brg"
{ fprintf(yyout, pfLABEL, mklbl(LEFT_CHILD(p)->place)); }
		break;
	case 65: /* or: expr */
		fprintf(stderr, "0x%p: line 248: or: expr\n",(void*)p);
#line 248 "minor.brg"
{ p->place = ++lbl; fprintf(yyout, pfDUP pfJNZ pfTRASH, mklbl(p->place), pfWORD); }
		break;
	case 66: /* expr: UMINUS(expr) */
		fprintf(stderr, "0x%p: line 249: expr: UMINUS(expr)\n",(void*)p);
#line 249 "minor.brg"
{ fprintf(yyout, pfNEG); }
		break;
	case 67: /* expr: NOT(expr) */
		fprintf(stderr, "0x%p: line 250: expr: NOT(expr)\n",(void*)p);
#line 250 "minor.brg"
{ fprintf(yyout, pfIMM pfEQ, 0); }
		break;
	case 68: /* expr: POW(expr,expr) */
		fprintf(stderr, "0x%p: line 251: expr: POW(expr,expr)\n",(void*)p);
#line 251 "minor.brg"
{lbl++; fprintf(yyout, ";POW\n"
		...
		break;
	case 69: /* expr: EQ(chars,ID) */
		fprintf(stderr, "0x%p: line 264: expr: EQ(chars,ID)\n",(void*)p);
#line 264 "minor.brg"
{ret = getOffset(p->SUB(1)->value.s);
		...
		break;
	case 70: /* expr: EQ(expr,ID) */
		fprintf(stderr, "0x%p: line 277: expr: EQ(expr,ID)\n",(void*)p);
#line 277 "minor.brg"
{ ret = getOffset(p->SUB(1)->value.s);
                        if(ret){
                          fprintf(yyout, pfIMM "push ebp\n"
		...
		break;
	case 71: /* expr: IND(expr,ID) */
		fprintf(stderr, "0x%p: line 285: expr: IND(expr,ID)\n",(void*)p);
#line 285 "minor.brg"
{ ret = getOffset(p->SUB(1)->value.s);
                        if(ret){
		...
		break;
	case 72: /* expr: EQ(expr,IND(expr,ID)) */
		fprintf(stderr, "0x%p: line 294: expr: EQ(expr,IND(expr,ID))\n",(void*)p);
#line 294 "minor.brg"
