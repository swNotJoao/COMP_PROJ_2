/*
generated at Mon May 18 14:19:45 2020
...
static short yynts_9[] = { yyif_NT, yyelif_NT, yyinstr_NT, 0 };
static short yynts_10[] = { yyelif_NT, yyif_NT, 0 };
static short yynts_11[] = { yyif_NT, yyinstr_NT, 0 };
static short yynts_12[] = { yycond_NT, yyinstr_NT, 0 };
static short yynts_13[] = { yychars_NT, yychars_NT, 0 };
static short yynts_14[] = { yyexpr_NT, yyexpr_NT, 0 };
...
/* 12 */	"instr: print",
/* 13 */	"instr: expr",
/* 14 */	"instr: FI(THEN(if,elif),instr)",
/* 15 */	"elif: ELIF(elif,if)",
/* 16 */	"elif: NIL",
/* 17 */	"instr: FI(THEN(if,NIL),instr)",
/* 18 */	"if: IF(cond,instr)",
/* 19 */	"instr: FI(THEN(instr,NIL),NIL)",
/* 20 */	"instr: IF(cond,instr)",
/* 21 */	"cond: expr",
/* 22 */	"print: PSTMT(ID)",
/* 23 */	"print: PSTMT(chars)",
/* 24 */	"print: PSTMT(expr)",
/* 25 */	"chars: CHARS(NIL,chars)",
/* 26 */	"chars: INT",
/* 27 */	"chars: CHAR",
/* 28 */	"chars: STR",
/* 29 */	"chars: CHARS(chars,chars)",
/* 30 */	"expr: CHARS(NIL,INT)",
/* 31 */	"expr: CHARS(NIL,CHAR)",
/* 32 */	"expr: ID",
/* 33 */	"expr: IND(expr,ID)",
/* 34 */	"expr: READOP",
/* 35 */	"expr: ADDR(ID)",
/* 36 */	"expr: ADDR(IND(expr,ID))",
/* 37 */	"expr: UMINUS(expr)",
...
static void yyclosure_expr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 21, c + 1, p->cost[yycond_NT]);
...
static void yylabel(NODEPTR_TYPE a, NODEPTR_TYPE u) {
	...
	case 33: /* PSTMT */
		...
		if (	/* print: PSTMT(ID) */
			...
			c = 1;
			yytrace(a, 22, c + 0, p->cost[yyprint_NT]);
		...
		yytrace(a, 23, c + 0, p->cost[yyprint_NT]);
		if (c + 0 < p->cost[yyprint_NT]) {
			p->cost[yyprint_NT] = c + 0;
			p->rule.yyprint = 2;
			yyclosure_print(a, c + 0);
		...
		yytrace(a, 24, c + 0, p->cost[yyprint_NT]);
		if (c + 0 < p->cost[yyprint_NT]) {
			p->cost[yyprint_NT] = c + 0;
			p->rule.yyprint = 3;
			yyclosure_print(a, c + 0);
	...
	case 63: /* READOP */
		/* expr: READOP */
		yytrace(a, 34, 0 + 0, p->cost[yyexpr_NT]);
	...
	case 257: /* INT */
		/* chars: INT */
		yytrace(a, 26, 0 + 0, p->cost[yychars_NT]);
	...
	case 258: /* CHAR */
		/* chars: CHAR */
		yytrace(a, 27, 0 + 0, p->cost[yychars_NT]);
		if (0 + 0 < p->cost[yychars_NT]) {
			p->cost[yychars_NT] = 0 + 0;
			p->rule.yychars = 3;
		}
	...
	case 260: /* STR */
		/* chars: STR */
		yytrace(a, 28, 0 + 0, p->cost[yychars_NT]);
		if (0 + 0 < p->cost[yychars_NT]) {
			p->cost[yychars_NT] = 0 + 0;
			p->rule.yychars = 4;
		}
	...
	case 272: /* IF */
		...
		yytrace(a, 18, c + 0, p->cost[yyif_NT]);
	...
	case 275: /* ELIF */
		...
		yytrace(a, 15, c + 0, p->cost[yyelif_NT]);
	...
	case 293: /* NIL */
		...
		/* elif: NIL */
		yytrace(a, 16, 2 + 0, p->cost[yyelif_NT]);
...
static void yykids(NODEPTR_TYPE p, int eruleno, NODEPTR_TYPE kids[]) {
	...
	switch (eruleno) {
	case 53: /* expr: POW(expr,expr) */
	case 50: /* expr: OR(expr,expr) */
	case 49: /* expr: AND(expr,expr) */
	...
	case 29: /* chars: CHARS(chars,chars) */
	case 20: /* instr: IF(cond,instr) */
	case 18: /* if: IF(cond,instr) */
	case 15: /* elif: ELIF(elif,if) */
	...
	case 1: /* file: PROGRAM(decls,main) */
		...
		break;
	case 35: /* expr: ADDR(ID) */
	case 34: /* expr: READOP */
	case 32: /* expr: ID */
	case 31: /* expr: CHARS(NIL,CHAR) */
	case 30: /* expr: CHARS(NIL,INT) */
	case 28: /* chars: STR */
	case 27: /* chars: CHAR */
	case 26: /* chars: INT */
	case 22: /* print: PSTMT(ID) */
	case 16: /* elif: NIL */
	case 9: /* instr: NIL */
	...
	case 12: /* instr: print */
		kids[0] = p;
		break;
	case 14: /* instr: FI(THEN(if,elif),instr) */
		kids[0] = LEFT_CHILD(LEFT_CHILD(p));
		kids[1] = RIGHT_CHILD(LEFT_CHILD(p));
		kids[2] = RIGHT_CHILD(p);
		break;
	case 17: /* instr: FI(THEN(if,NIL),instr) */
		kids[0] = LEFT_CHILD(LEFT_CHILD(p));
		kids[1] = RIGHT_CHILD(p);
		break;
	case 36: /* expr: ADDR(IND(expr,ID)) */
	case 19: /* instr: FI(THEN(instr,NIL),NIL) */
		kids[0] = LEFT_CHILD(LEFT_CHILD(p));
		break;
	case 54: /* expr: EQ(expr,ID) */
	case 52: /* expr: NOT(expr) */
	case 51: /* expr: UMINUS(expr) */
	case 37: /* expr: UMINUS(expr) */
	case 33: /* expr: IND(expr,ID) */
	case 24: /* print: PSTMT(expr) */
	case 23: /* print: PSTMT(chars) */
		kids[0] = LEFT_CHILD(p);
		break;
	default:
...
#line 85 "minor.brg"
		...
		break;
	case 14: /* instr: FI(THEN(if,elif),instr) */
		fprintf(stderr, "0x%p: line 87: instr: FI(THEN(if,elif),instr)\n",(void*)p);
#line 87 "minor.brg"
{ fprintf(yyout, pfLABEL,
                                      mklbl(LEFT_CHILD(p)->SUB(0)->place)); }
		break;
	case 15: /* elif: ELIF(elif,if) */
		fprintf(stderr, "0x%p: line 89: elif: ELIF(elif,if)\n",(void*)p);
#line 89 "minor.brg"
{ p->place = ++lbl; fprintf(yyout, pfJMP pfLABEL,
				                mklbl(RIGHT_CHILD(p)->place), mklbl(p->place)); }
		break;
	case 16: /* elif: NIL */
		fprintf(stderr, "0x%p: line 91: elif: NIL\n",(void*)p);
#line 91 "minor.brg"
{}
		break;
	case 17: /* instr: FI(THEN(if,NIL),instr) */
		fprintf(stderr, "0x%p: line 93: instr: FI(THEN(if,NIL),instr)\n",(void*)p);
#line 93 "minor.brg"
{ fprintf(yyout, pfLABEL,
                                      mklbl(LEFT_CHILD(p)->SUB(0)->place)); }
		break;
	case 18: /* if: IF(cond,instr) */
		fprintf(stderr, "0x%p: line 95: if: IF(cond,instr)\n",(void*)p);
#line 95 "minor.brg"
{p->place = ++lbl; fprintf(yyout, pfJMP pfLABEL,
				            mklbl(p->place), mklbl(LEFT_CHILD(p)->place)); }
		break;
	case 19: /* instr: FI(THEN(instr,NIL),NIL) */
		fprintf(stderr, "0x%p: line 98: instr: FI(THEN(instr,NIL),NIL)\n",(void*)p);
#line 98 "minor.brg"
{/*Simple if*/}
		break;
	case 20: /* instr: IF(cond,instr) */
		fprintf(stderr, "0x%p: line 99: instr: IF(cond,instr)\n",(void*)p);
#line 99 "minor.brg"
{ fprintf(yyout, pfLABEL, mklbl(LEFT_CHILD(p)->place)); }
		break;
	case 21: /* cond: expr */
		fprintf(stderr, "0x%p: line 100: cond: expr\n",(void*)p);
#line 100 "minor.brg"
{ p->place = ++lbl; fprintf(yyout, pfJZ, mklbl(p->place)); }
		break;
	case 22: /* print: PSTMT(ID) */
		fprintf(stderr, "0x%p: line 102: print: PSTMT(ID)\n",(void*)p);
#line 102 "minor.brg"
{
                  switch(p->SUB(0)->info){
                    case tINT:
		...
		break;
	case 23: /* print: PSTMT(chars) */
		fprintf(stderr, "0x%p: line 114: print: PSTMT(chars)\n",(void*)p);
#line 114 "minor.brg"
{
		...
		break;
	case 24: /* print: PSTMT(expr) */
		fprintf(stderr, "0x%p: line 122: print: PSTMT(expr)\n",(void*)p);
#line 122 "minor.brg"
{fprintf(yyout, pfTEXT pfCALL pfTRASH, "_printi", 4);}
		break;
	case 25: /* chars: CHARS(NIL,chars) */
		fprintf(stderr, "0x%p: line 124: chars: CHARS(NIL,chars)\n",(void*)p);
#line 124 "minor.brg"
{}
		break;
	case 26: /* chars: INT */
		fprintf(stderr, "0x%p: line 125: chars: INT\n",(void*)p);
#line 125 "minor.brg"
{ tmpStr[tmpStrInd++] = p->value.i; toPrint = tmpStr;}
		break;
	case 27: /* chars: CHAR */
		fprintf(stderr, "0x%p: line 126: chars: CHAR\n",(void*)p);
#line 126 "minor.brg"
{ tmpStr[tmpStrInd++] = p->value.i; toPrint = tmpStr;}
		break;
	case 28: /* chars: STR */
		fprintf(stderr, "0x%p: line 127: chars: STR\n",(void*)p);
#line 127 "minor.brg"
{ strcpy(&tmpStr[tmpStrInd], p->value.s); tmpStrInd = tmpStrInd + strlen(p->value.s); toPrint = tmpStr;}
		break;
	case 29: /* chars: CHARS(chars,chars) */
		fprintf(stderr, "0x%p: line 128: chars: CHARS(chars,chars)\n",(void*)p);
#line 128 "minor.brg"
{}
		break;
	case 30: /* expr: CHARS(NIL,INT) */
		fprintf(stderr, "0x%p: line 130: expr: CHARS(NIL,INT)\n",(void*)p);
#line 130 "minor.brg"
{fprintf(yyout, pfIMM, p->SUB(1)->value.i);}
		break;
	case 31: /* expr: CHARS(NIL,CHAR) */
		fprintf(stderr, "0x%p: line 131: expr: CHARS(NIL,CHAR)\n",(void*)p);
#line 131 "minor.brg"
{fprintf(yyout, pfIMM, p->SUB(1)->value.i); }
		break;
	case 32: /* expr: ID */
		fprintf(stderr, "0x%p: line 132: expr: ID\n",(void*)p);
#line 132 "minor.brg"
{ fprintf(yyout, pfADDRV, p->value.s); }
		break;
	case 33: /* expr: IND(expr,ID) */
		fprintf(stderr, "0x%p: line 133: expr: IND(expr,ID)\n",(void*)p);
#line 133 "minor.brg"
{ fprintf(yyout, pfADDR pfADD, p->SUB(1)->value.s); }
		break;
	case 34: /* expr: READOP */
		fprintf(stderr, "0x%p: line 134: expr: READOP\n",(void*)p);
#line 134 "minor.brg"
{fprintf(yyout, pfCALL pfPUSH, "_readi"); }
		break;
	case 35: /* expr: ADDR(ID) */
		fprintf(stderr, "0x%p: line 136: expr: ADDR(ID)\n",(void*)p);
#line 136 "minor.brg"
{ fprintf(yyout, pfADDR, p->SUB(0)->value.s); }
		break;
	case 36: /* expr: ADDR(IND(expr,ID)) */
		fprintf(stderr, "0x%p: line 137: expr: ADDR(IND(expr,ID))\n",(void*)p);
#line 137 "minor.brg"
{ switch(p->SUB(0)->SUB(1)->info){
		...
		break;
	case 37: /* expr: UMINUS(expr) */
		fprintf(stderr, "0x%p: line 147: expr: UMINUS(expr)\n",(void*)p);
#line 147 "minor.brg"
{ fprintf(yyout, pfIMM pfMUL, -1); }
		break;
	case 38: /* expr: ADD(expr,expr) */
		fprintf(stderr, "0x%p: line 148: expr: ADD(expr,expr)\n",(void*)p);
#line 148 "minor.brg"
{ fprintf(yyout, pfADD); }
		break;
	case 39: /* expr: SUB(expr,expr) */
		fprintf(stderr, "0x%p: line 149: expr: SUB(expr,expr)\n",(void*)p);
#line 149 "minor.brg"
{ fprintf(yyout, pfSUB); }
		break;
	case 40: /* expr: MUL(expr,expr) */
		fprintf(stderr, "0x%p: line 150: expr: MUL(expr,expr)\n",(void*)p);
#line 150 "minor.brg"
{ fprintf(yyout, pfMUL); }
		break;
	case 41: /* expr: DIV(expr,expr) */
		fprintf(stderr, "0x%p: line 151: expr: DIV(expr,expr)\n",(void*)p);
#line 151 "minor.brg"
{ fprintf(yyout, pfDIV); }
		break;
	case 42: /* expr: MOD(expr,expr) */
		fprintf(stderr, "0x%p: line 152: expr: MOD(expr,expr)\n",(void*)p);
#line 152 "minor.brg"
{ fprintf(yyout, pfMOD); }
		break;
	case 43: /* expr: ISEQ(expr,expr) */
		fprintf(stderr, "0x%p: line 153: expr: ISEQ(expr,expr)\n",(void*)p);
#line 153 "minor.brg"
{ fprintf(yyout, pfEQ); }
		break;
	case 44: /* expr: NE(expr,expr) */
		fprintf(stderr, "0x%p: line 154: expr: NE(expr,expr)\n",(void*)p);
#line 154 "minor.brg"
{ fprintf(yyout, pfNE); }
		break;
	case 45: /* expr: LT(expr,expr) */
		fprintf(stderr, "0x%p: line 155: expr: LT(expr,expr)\n",(void*)p);
#line 155 "minor.brg"
{ fprintf(yyout, pfLT); }
		break;
	case 46: /* expr: LE(expr,expr) */
		fprintf(stderr, "0x%p: line 156: expr: LE(expr,expr)\n",(void*)p);
#line 156 "minor.brg"
{ fprintf(yyout, pfLE); }
		break;
	case 47: /* expr: GT(expr,expr) */
		fprintf(stderr, "0x%p: line 157: expr: GT(expr,expr)\n",(void*)p);
#line 157 "minor.brg"
{ fprintf(yyout, pfGT); }
		break;
	case 48: /* expr: GE(expr,expr) */
		fprintf(stderr, "0x%p: line 158: expr: GE(expr,expr)\n",(void*)p);
#line 158 "minor.brg"
{ fprintf(yyout, pfGE); }
		break;
	case 49: /* expr: AND(expr,expr) */
		fprintf(stderr, "0x%p: line 159: expr: AND(expr,expr)\n",(void*)p);
#line 159 "minor.brg"
{ fprintf(yyout, pfAND); }
		break;
	case 50: /* expr: OR(expr,expr) */
		fprintf(stderr, "0x%p: line 160: expr: OR(expr,expr)\n",(void*)p);
#line 160 "minor.brg"
{ fprintf(yyout, pfOR); }
		break;
	case 51: /* expr: UMINUS(expr) */
		fprintf(stderr, "0x%p: line 161: expr: UMINUS(expr)\n",(void*)p);
#line 161 "minor.brg"
{ fprintf(yyout, pfNEG); }
		break;
	case 52: /* expr: NOT(expr) */
		fprintf(stderr, "0x%p: line 162: expr: NOT(expr)\n",(void*)p);
#line 162 "minor.brg"
{ fprintf(yyout, pfIMM pfEQ, 0); }
		break;
	case 53: /* expr: POW(expr,expr) */
		fprintf(stderr, "0x%p: line 163: expr: POW(expr,expr)\n",(void*)p);
#line 163 "minor.brg"
{lbl++; fprintf(yyout, ";POW\n"
		...
		break;
	case 54: /* expr: EQ(expr,ID) */
		fprintf(stderr, "0x%p: line 176: expr: EQ(expr,ID)\n",(void*)p);
#line 176 "minor.brg"
